...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			pagestat: 'Отображается с {from} по {to} из {total} элементов',
			pagetext: 'Страница',
			outof: 'из',
			findtext: 'Найти',
			procmsg: 'Обработка данных, пожалуйста, подождите ...',
			query: '',
			qtype: '',
			nomsg: 'Нет данных',
		...
		if (p.title) {
			...
			if (p.showTableToggleBtn) {
				$(g.mDiv).append('<div class="ptogtitle" title="Свернуть/Развернуть таблицу"><span></span></div>');
		...
		if ($('th', g.hDiv).length) {
			...
			$(g.nBtn).addClass('nBtn')
				.html('<div></div>')
				.attr('title', 'Скрыть/Показать поля')
				.click(function () {
					$(g.nDiv).toggle();
					return true;
				}
